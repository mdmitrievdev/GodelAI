...
- **Acceptance Criteria:**
  - [ ] Covers `bin/`, `obj/`, `node_modules/`, `dist/`, `.env`, `*.user`, `.vs/`, `.idea/`
  - [ ] Includes Docker ignore patterns (`docker-compose.override.yml`)
  - [ ] File at repo root
- **Output:** `.gitignore`
...
- **Acceptance Criteria:**
  - [ ] `backend/PRReviewAssistant.sln` exists
  - [ ] `backend/src/PRReviewAssistant.API/PRReviewAssistant.API.csproj` targets `net9.0`
  - [ ] `backend/tests/PRReviewAssistant.Tests/PRReviewAssistant.Tests.csproj` targets `net9.0`
  - [ ] NuGet packages installed: MediatR, FluentValidation.AspNetCore, Npgsql.EntityFrameworkCore.PostgreSQL, Swashbuckle.AspNetCore, NUlid, Microsoft.EntityFrameworkCore.Design
  - [ ] Test packages: Moq, FluentAssertions, Microsoft.EntityFrameworkCore.InMemory
...
- **Acceptance Criteria:**
  - [ ] `frontend/` created with Vite React TS template
  - [ ] `tsconfig.json` has `strict: true`, `noUncheckedIndexedAccess: true`
  - [ ] Dependencies: `react-router-dom`, `axios`
  - [ ] Dev dependencies: testing-library packages, jest, eslint, prettier
...
- **Acceptance Criteria:**
  - [ ] App starts and `/health` returns `200 OK`
...
- **Acceptance Criteria:**
  - [ ] `Review` has: `Id` (string/ULID), `CodeDiff`, `Language`, `PrUrl?`, `CreatedAt` (UTC), `Findings` (nav prop)
  - [ ] `Finding` has all fields per SRS including `Confidence` (0–100) and `SuggestedFix?`
  - [ ] `FindingCategory` enum: Bug, Naming, Performance, Security, CodeStyle, BestPractice
  - [ ] `FindingSeverity` enum: Critical, Warning, Info
...
- **Acceptance Criteria:**
  - [ ] Fields: `Id` (string), `UseMockAi` (bool, default true), `AiModel` (string, default "mock")
...
- **Acceptance Criteria:**
  - [ ] `CreateReviewRequest`, `UpdateSettingsRequest` (request records)
  - [ ] `ReviewDetailResponse`, `ReviewListItem`, `FindingDto`, `ReviewSummaryDto`, `AppSettingsResponse` (response records)
  - [ ] `PaginatedList<T>` record with `Items`, `TotalCount`, `Page`, `PageSize`
  - [ ] `ApiResponse<T>` with `Data` and `Error` (ProblemDetails?) properties
  - [ ] All use `record` syntax, `IReadOnlyList<T>` for collections
  - [ ] String IDs, nullable where specified
...
- **Acceptance Criteria:**
  - [ ] `IReviewRepository`: `CreateAsync`, `GetByIdAsync`, `GetAllAsync` (paginated), `DeleteAsync` — all with `CancellationToken`
  - [ ] `ISettingsRepository`: `GetAsync`, `UpdateAsync` — all with `CancellationToken`
  - [ ] `IAiAnalysisService`: `AnalyzeAsync(string codeDiff, string language, CancellationToken ct)` returning `Task<IReadOnlyList<Finding>>`
  - [ ] XML doc comments on all methods
...
- **Acceptance Criteria:**
  - [ ] `AppDbContext` inherits `DbContext`, has `DbSet<Review>`, `DbSet<Finding>`, `DbSet<AppSettings>`
  - [ ] Fluent API: primary keys, max lengths, required fields, FK relationships, cascade delete on `Review→Findings`
  - [ ] `AppSettings` seeded with default values
...
- **Acceptance Criteria:**
  - [ ] `ReviewRepository` injects `AppDbContext`
  - [ ] `GetAllAsync` supports pagination with `Skip`/`Take`, returns `(IReadOnlyList<ReviewListItem>, int totalCount)`
  - [ ] `GetByIdAsync` includes `Finding` navigation property
  - [ ] `DeleteAsync` cascades to findings
  - [ ] `AsNoTracking()` for read queries
  - [ ] `SettingsRepository` ensures singleton — creates default if missing
...
- **Acceptance Criteria:**
  - [ ] Implements `IAiAnalysisService`
  - [ ] Returns 3–8 findings per analysis
  - [ ] Findings vary by language and diff length
  - [ ] Each finding has realistic `Title`, `Description`, `Suggestion`, `LineReference`, `Confidence` (40–98), `SuggestedFix`
  - [ ] All 6 categories and 3 severity levels represented across calls
  - [ ] Simulates 200–800ms delay (configurable)
  - [ ] Generates ULID IDs
...
- **Acceptance Criteria:**
  - [ ] Migration applies cleanly to a fresh PostgreSQL database
  - [ ] `AppSettings` seed data included
  - [ ] Tables: `Reviews`, `Findings`, `AppSettings`
...
- **Acceptance Criteria:**
  - [ ] `CreateReviewRequestValidator`: `CodeDiff` not empty, max 50000 chars; `Language` not empty, must be in supported list; `PrUrl` optional but valid URL format if provided
  - [ ] `UpdateSettingsRequestValidator`: `AiModel` not empty, max 100 chars
...
- **Acceptance Criteria:**
  - [ ] Command implements `IRequest<ReviewDetailResponse>`
  - [ ] Handler: validates → calls `IAiAnalysisService.AnalyzeAsync` → creates `Review` with ULID → saves via repository → maps to `ReviewDetailResponse`
  - [ ] POST `/api/v1/reviews` with valid diff returns `201` with response containing ≥3 findings
  - [ ] `CancellationToken` passed through all calls
...
- **Acceptance Criteria:**
  ...
  - [ ] `CodeSnippet` truncated to 80 chars
...
- **Acceptance Criteria:**
  - [ ] Returns `ReviewDetailResponse` with findings and summary
  - [ ] Invalid ID → returns null (endpoint converts to 404 ProblemDetails)
  - [ ] Summary: `TotalFindings`, `CriticalCount`, `WarningCount`, `InfoCount`, `AverageConfidence`
...
- **Acceptance Criteria:**
  ...
  - [ ] Valid ID → deletes review + findings, returns `true`
...
- **Acceptance Criteria:**
  - [ ] `GetSettingsQuery` → returns `AppSettingsResponse`
  - [ ] `UpdateSettingsCommand` → updates settings, returns `AppSettingsResponse`
  - [ ] Settings singleton ensured (creates default if missing)
...
- **Acceptance Criteria:**
  - [ ] `ReviewEndpoints`: POST, GET (list), GET (by id), DELETE
  - [ ] `SettingsEndpoints`: GET, PUT
  - [ ] All routes prefixed `/api/v1/`
  - [ ] Responses wrapped in `ApiResponse<T>`
  - [ ] 404 returned with ProblemDetails when entity not found
...
- **Acceptance Criteria:**
  - [ ] Registers: MediatR, FluentValidation, EF Core, repositories, `IAiAnalysisService` → `MockAiAnalysisService`
  - [ ] Middleware: exception handler, CORS, Swagger (dev only)
  - [ ] Maps: `ReviewEndpoints`, `SettingsEndpoints`, `/health`
  - [ ] `dotnet build` passes; `dotnet run` starts successfully; all endpoints respond
...
- **Acceptance Criteria:**
  - [ ] All interfaces match copilot-instructions.md TypeScript Interfaces section exactly
  - [ ] Types: `FindingCategory`, `FindingSeverity` as string unions
  - [ ] `ApiResponse<T>` interface: `{ data: T | null; error: ProblemDetails | null }`
...
- **Acceptance Criteria:**
  ...
  - [ ] Methods: `createReview`, `getReviews`, `getReviewById`, `deleteReview`, `getSettings`, `updateSettings`
  - [ ] All return typed responses
  - [ ] Error interceptor normalizes to `{ message: string; code?: string }`
  - [ ] No `any` types
...
- **Acceptance Criteria:**
  ...
  - [ ] Top bar with "Admin" badge and "Back to Main" link
...
- **Acceptance Criteria:**
  - [ ] `/` → `HomePage` (MainLayout)
  - [ ] `/review` → `ReviewPage` (MainLayout)
  - [ ] `/review/:id` → `ReviewResultPage` (MainLayout)
  - [ ] `/admin/history` → `HistoryPage` (AdminLayout)
  - [ ] `/admin/settings` → `AdminSettingsPage` (AdminLayout)
  - [ ] Unknown routes → redirect to `/`
  - [ ] Pages can be placeholder stubs initially
...
- **Acceptance Criteria:**
  - [ ] `ErrorMessage`: accepts `message: string`, renders styled error alert
  - [ ] `LoadingSpinner`: visual spinner with optional `message` prop
  - [ ] `SeverityBadge`: renders colored badge (red/orange/blue) based on `FindingSeverity`
  - [ ] `ConfidenceBadge`: renders percentage badge with color gradient
...
- **Acceptance Criteria:**
  - [ ] `useReviews(page, pageSize)`: returns `{ data: PaginatedList<ReviewListItem> | null, loading, error, refetch }`
  - [ ] `useReviewDetail(id)`: returns `{ data: ReviewDetailResponse | null, loading, error }`
  - [ ] `useSettings()`: returns `{ data: AppSettingsResponse | null, loading, error, updateSettings }`
  - [ ] `useCreateReview()`: returns `{ submit, loading, error }`
  - [ ] `useDeleteReview()`: returns `{ deleteReview, loading, error }`
  - [ ] All normalize errors at the API boundary
...
- **Acceptance Criteria:**
  - [ ] `FindingCard`: displays finding title, severity badge, category, confidence, description, suggestion; toggle for `SuggestedFixPanel`
  - [ ] `FindingsList`: accepts `findings[]` + `filters`, renders filtered `FindingCard` list grouped by category
  - [ ] `ReviewSummary`: displays total, counts per severity, average confidence
  - [ ] `SuggestedFixPanel`: collapsible panel showing `suggestedFix` code
...
- **Acceptance Criteria:**
  ...
  - [ ] Feature highlights (categories, severity, history)
  - [ ] Admin link to `/admin/history`
...
- **Acceptance Criteria:**
  - [ ] Textarea for code diff (placeholder text)
  - [ ] Language dropdown with 7 supported languages
  - [ ] Optional PR URL input
  - [ ] Client-side validation: non-empty diff, language selected
  - [ ] On submit: calls `useCreateReview`, shows `LoadingSpinner`, navigates to `/review/:id` on success
  - [ ] Errors shown via `ErrorMessage`
...
- **Acceptance Criteria:**
  - [ ] Loads review by `id` from URL params
  - [ ] Shows `ReviewSummary` at top
  - [ ] Severity filter buttons (All, Critical, Warning, Info)
  - [ ] Category filter dropdown (All + 6 categories)
  - [ ] `FindingsList` renders filtered findings
  - [ ] Original diff displayed in code block (collapsible)
...
- **Acceptance Criteria:**
  - [ ] Table/list: date, language, snippet (80 chars), finding counts
...
- **Acceptance Criteria:**
  ...
  - [ ] Consistent severity colors (red, orange, blue)
...
- **Acceptance Criteria:**
  - [ ] 3 services: `frontend` (port 3000), `backend` (port 8080), `db` (port 5432)
...
- **Acceptance Criteria:**
  - [ ] Stage 1: `dotnet/sdk:9.0`, restore, build, test, publish
  - [ ] Stage 2: `dotnet/aspnet:9.0`, HEALTHCHECK, ENTRYPOINT
  - [ ] Tests run during build — fail stops build
  - [ ] `.dockerignore` present
...
- **Acceptance Criteria:**
  ...
  - [ ] `docker` job: compose build + up + health check (needs frontend+backend)
...
- **Acceptance Criteria:**
  - [ ] `CreateReviewCommandHandler` tests: valid input, empty diff
  - [ ] `GetReviewsQueryHandler` tests: pagination, empty set
  - [ ] `CreateReviewRequestValidator` tests: empty diff fails, valid passes
  - [ ] `MockAiAnalysisService` tests: returns findings, varies by language
...
- **Acceptance Criteria:**
  - [ ] Follows template from `5-documentation-templates.md`
  - [ ] All sections filled: overview, getting started, API endpoints, AI stats placeholder
  - [ ] Docker and local run instructions accurate
- **Output:** `README.md`
...
- **Acceptance Criteria:**
  - [ ] Prompts that worked well (≥3)
  - [ ] Prompts that didn't work (≥2)
  - [ ] Effective patterns
  - [ ] Key metrics table
  - [ ] Tools assessment
...
- **Acceptance Criteria:**
  ...
  - [ ] Full user journey works: submit → view → history → delete
  - [ ] Frontend builds with 0 errors/warnings
  - [ ] Backend builds with 0 errors/warnings
  - [ ] All prompt logs exist in `prompts/`
  - [ ] No `console.log`, `debugger`, or TODO in code
